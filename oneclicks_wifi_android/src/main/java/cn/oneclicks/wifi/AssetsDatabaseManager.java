...
public class AssetsDatabaseManager {  
    ...
    private static String databasepath = "/data/data/cn.oneclicks.wifi/database"; // %s is packageName  
    ...
    //Manual update DBVersion when DB file changed
    ...
    public SQLiteDatabase getDatabase(String dbfile) {  
        if(databases.get(dbfile) != null){  
            ...
            Log.v("DBVersion",Integer.toString(databases.get(dbfile).getVersion()));  
            if(databases.get(dbfile).getVersion()==DBVersion)
        ...
        int flag = dbs.getInt(dbfile, 0); // Get Database file flag, if true means this database file was copied and valid  
        
        if(flag!=DBVersion || !file.exists()){  
            file = new File(spath);  
            ...
            dbs.edit().putInt(dbfile, DBVersion).commit();  
        }  
        ...
        Log.v("DBVersion2",Integer.toString(databases.get(dbfile).getVersion())); 
        return db;  
    }  
